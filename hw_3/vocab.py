...
class Vocab:
    ...
    @staticmethod
    def from_lines(lines, freq_threshold=3, bos="_BOS_", eos="_EOS_", unk='_UNK_'):
        """
        Создаёт словарь, исключая токены, которые встречаются реже freq_threshold раз.
        """
        all_tokens = []
        for line in lines:
        ...
        main_tokens = [t for t, c in freqs.items() 
                       if c >= freq_threshold and t not in (bos, eos, unk)]
        main_tokens = sorted(main_tokens)
        final_list = [bos, eos, unk] + main_tokens
        return Vocab(final_list, bos, eos, unk)
